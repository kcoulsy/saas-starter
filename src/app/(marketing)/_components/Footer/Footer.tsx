...
const LinkGroup = ({ children, header }: PropsWithChildren<{ header?: string | React.ReactNode }>) => {
  return (
...
const NavLink = ({ link, label }: { link: string; label: string }) => {
  return (
    <div>
      <li>
        <a href={link} className="inline-block mb-2 text-base leading-loose text-body-color hover:text-primary">
          {label}
        </a>
      </li>
    </div>
        ...
        <div className="flex flex-wrap -mx-4">
          ...
          <div className="w-full px-4 sm:w-1/2 lg:w-3/12">
            <div className="w-full mb-10">
              ...
              <p className="text-base text-body-color"> © {new Date().getFullYear()} TailGrids </p>
            </div>
          </div>
        </div>
      </div>
